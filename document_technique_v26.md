...
**Version :** 26.0 — **✅ Jauge de batterie fermée** (LM3914, 10 segments 2R/3J/5V, bargraph OSX10201-GYR1, construite dans `Alimentation.kicad_sch`) ; rattrapage documentaire des fermetures de la session du 11 septembre (Injection ×6 canaux, Alarme court-circuit bus, Hub/Alerte_Incohérence, coupure V12_Rail, Bloc_Test_Left validé, nommage Bloc_Test_Brake) ; **⚠️ conflit identifié et non résolu** sur le Chantier #7/R8 (revalidation RL_x) — deux approches concurrentes en attente d'arbitrage, voir §10.1
**Version :** 23.0 — **✅ VEHICLE_LEFT validé** avec références réelles confirmées par BOM (remplace la numérotation provisoire de la v22) ; **TRAILER_LEFT** : références réelles suivantes confirmées par BOM avant câblage (patte-à-patte à venir)
**Version :** 22.0 — **✅ Canal_TAIL fermé et validé** : résolution du point §10 (oscillation Q_TAIL_INJECT) par déplacement de la mesure d'U2A en amont de Q11 via un pont diviseur haute impédance (R37/R38) ; correction du fil manquant Q11 pin 3 → `Q11_Source` ; Canal_TAIL sert désormais de modèle figé pour la réplication des 5 autres canaux
**Date :** Septembre 2026
...
**R8 — Revalidation systématique des composants dimensionnés empiriquement.** Tout composant dont la valeur est fixée par une plage indicative plutôt qu'un calcul définitif (ex. shunts RSH_x/RSH2_x, résistances de charge RL_x, calibre PTC, tenue TVS) doit être revalidé par un calcul réel (courant nominal mesuré, P = I²×R, marge de sécurité) **avant fabrication**, canal par canal — jamais assumé correct simplement parce qu'il figure dans une plage de brouillon initial. Voir §10 pour la checklist correspondante.
...
| Structure de feuilles KiCad du Bloc 3 : 6 feuilles séparées par canal (plutôt qu'une feuille unique avec bus), sous-blocs en **vraies sous-feuilles hiérarchiques (Option 2)** — décision finale, remplace l'Option 1 (rectangles graphiques) initialement recommandée | Utilisateur | ✅ Acceptée — voir §7.1 (révisé une 2ᵉ fois) |
| Étape méthodologique systématique de revalidation par calcul réel (P = I²×R, marge) pour tout composant dimensionné par une plage empirique (shunts, RL_x, PTC, TVS), avant fabrication, canal par canal | Utilisateur | ✅ Acceptée — voir R8 et §10.1 (nouvelle checklist) |
| PTC dédié côté VEHICULE (PTC_TAIL_VEH), en série avant RL_TAIL, contre un courant élevé venant du véhicule | Claude | ❌ Refusée — ne passe pas le test d'utilité (R3) : inutile contre un load dump (réaction PTC trop lente vs transitoire de quelques ms — c'est le TVS qui protège déjà), ne règle pas vraiment un branchement 24V accidentel (hors cahier des charges F2/§2), et le seul scénario résiduel plausible (survoltage soutenu) est déjà atténué par le choix d'une résistance de charge de qualité (R4) |
| Support AUX pour remorque triple essieu (~20-24 A, 6 résistances RL_AUX en parallèle + dissipateur MOSFET) | Utilisateur / Claude | ⏸️ En suspens — version future de l'appareil. Version actuelle limitée à tandem/2 essieux |
| Jauge de batterie : passage de 5 à 10 segments (2 rouge / 3 jaune / 5 vert), bargraph OSX10201-GYR1 déjà importé | Utilisateur | ✅ Acceptée — voir §7, §9, Annexe A.6 (mise à jour) |
...
- **Jauge de batterie à 10 segments** (rangée du bas, avec les DEL d'anomalie) : bargraph LED 10 segments en un seul boîtier compact (OSX10201-GYR1), couleurs **2 Rouge, 3 Jaune, 5 Vert** (de bas à haut, mode « bar » — les segments s'allument en cascade selon le niveau de charge). Pilotée par une puce **LM3914** (driver analogique dédié, DIP-18, socket turned-pin R7), avec un potentiomètre de calibration (Bourns 3296W-1-102LF, 1 kΩ, 25 tours) pour ajuster la plage de tension affichée à la chimie exacte du pack. **✅ Fermée** — construite dans `Alimentation.kicad_sch` (et non `Alarmes_Buzzer_sch` comme envisagé initialement), voir Annexe A.6 pour le détail complet du câblage.
...
**Décision d'architecture de feuille (Bloc 3 — RÉVISÉE) :** après avoir constaté la densité visuelle du câblage complet d'un seul canal (§7.2), la décision initiale (une seule feuille « Canal_Test » regroupant les 6 canaux via un bus hiérarchique) est **remplacée** par une structure à **6 feuilles hiérarchiques séparées**, une par canal : `Canal_TAIL`, `Canal_BRAKE`, `Canal_LEFT`, `Canal_RIGHT`, `Canal_BACKUP`, `Canal_AUX` — chacune avec son propre symbole/rectangle sur la page racine.

**Décision de câblage inter-feuilles (RÉVISÉE) :** le bus hiérarchique devient inutile avec cette structure — chaque feuille n'a que ses 2 signaux propres à faire sortir. `VEH_x` et `TRAIL_x` sont donc de simples **étiquettes hiérarchiques individuelles** (2 par feuille), pas des membres de bus. La masse (`GND`) et le rail de référence des comparateurs (`V_REF`) restent des **étiquettes globales** communes comme avant, inchangées par cette révision.

**Sous-blocs à l'intérieur de chaque feuille de canal (Option 2 retenue — RÉVISÉ à nouveau) :** après réflexion, l'utilisateur a choisi de pousser la structure hiérarchique un niveau plus loin que l'Option 1 initialement recommandée. Chaque sous-bloc (VEHICLE, TRAILER, Bloc de Test) est une **véritable sous-feuille KiCad**, imbriquée sous la feuille du canal (ex. `Canal_TAIL` contient les sous-feuilles `VEHICLE_TAIL`, `TRAILER_TAIL`, `Bloc_Test_Tail`). Raison du choix : une séparation purement graphique (Option 1) ne garantit rien structurellement — KiCad ne force aucune discipline de câblage entre les rectangles ; une vraie sous-feuille rend la frontière physique et infranchissable sans étiquette hiérarchique explicite, ce qui correspond mieux à la préférence de l'utilisateur pour une structure rigoureuse.

**Conséquence sur les étiquettes :** les signaux qui traversent la frontière entre sous-blocs ne sont donc **pas** des étiquettes locales comme documenté initialement en §7.2.1, mais bien des **étiquettes hiérarchiques**, une paire de pattes importée à chaque frontière franchie (via la fonction « synchroniser les pins de feuille » de KiCad). Pour le canal TAIL, seulement **5 signaux** traversent réellement une frontière :
- `Court_Tail_Vehic`, `Ouvert_Tail_Vehic` (VEHICLE_TAIL → Bloc_Test_Tail)
- `Court_Tail_Trailer`, `Ouvert_Tail_Trailer`, `Btn_Tail_Etat` (TRAILER_TAIL → Bloc_Test_Tail)

`GND`, `V_Ref`, `Mode_Vehicule`, `Mode_Trailer` restent des étiquettes **globales**, inchangées par cette structure — elles traversent tous les niveaux de hiérarchie automatiquement, aucune manipulation requise.
...
> Le canal TAIL sert de modèle complet. Les 5 autres canaux (BRAKE, LEFT, RIGHT, BACKUP, AUX) reprendront exactement la même structure — seule la référence change (ex. `_BRAKE` au lieu de `_TAIL`). **✅ CANAL_TAIL VALIDÉ (v22)** — les 3 sous-feuilles (`VEHICLE_TAIL`, `TRAILER_TAIL`, `Bloc_Test_Tail`) sont câblées, vérifiées par capture d'écran, et closes. Le point ouvert §10 (oscillation Q_TAIL_INJECT) est résolu (voir note ci-dessous). Ce canal sert désormais de référence figée pour la réplication des 5 autres.
...
|---|---|---|---|
| `VEH_TAIL` | Hiérarchique | VEHICLE_TAIL ↔ Canal_TAIL ↔ page racine | Fil vers le port physique VEHICLE |
| `TRAIL_TAIL` | Hiérarchique | TRAILER_TAIL ↔ Canal_TAIL ↔ page racine | Fil vers le port physique TRAILER |
...
| `Court_Tail_Vehic` | Hiérarchique (VEHICLE_TAIL → Bloc_Test_Tail) | VEHICLE_TAIL / Bloc_Test_Tail | Sortie comparateur seuil haut, côté VEHICLE |
| `Ouvert_Tail_Vehic` | Hiérarchique (VEHICLE_TAIL → Bloc_Test_Tail) | VEHICLE_TAIL / Bloc_Test_Tail | Sortie comparateur seuil bas, côté VEHICLE |
| `Court_Tail_Trailer` | Hiérarchique (TRAILER_TAIL → Bloc_Test_Tail) | TRAILER_TAIL / Bloc_Test_Tail | Sortie comparateur seuil haut, côté TRAILER |
| `Ouvert_Tail_Trailer` | Hiérarchique (TRAILER_TAIL → Bloc_Test_Tail) | TRAILER_TAIL / Bloc_Test_Tail | Sortie comparateur seuil bas, côté TRAILER |
| `Btn_Tail_Etat` | Hiérarchique (TRAILER_TAIL → Bloc_Test_Tail) | TRAILER_TAIL / Bloc_Test_Tail | État du bouton-poussoir (haut = enclenché) |
| `Court_V_Actif`, `Court_T_Actif` | Locale (nœuds internes, restent dans Bloc_Test_Tail) | Bloc_Test_Tail | Sorties des portes ET intermédiaires |
| `Ouvert_V_Actif`, `Ouvert_T_Actif` | Locale (nœuds internes) | Bloc_Test_Tail | Sorties des portes ET intermédiaires |
| `Court_Tail`, `Ouvert_Tail`, `Bleu_Tail` | Locale (nœuds finaux) | Bloc_Test_Tail | Signaux combinés finaux, avant pilotage DEL |
| `Not_Btn_Tail` | Locale (nœud interne) | Bloc_Test_Tail | Inversion de `BTN_TAIL_ETAT` |

#### 7.2.2 Composants — sous-bloc VEHICLE (canal TAIL)

| Réf. | Composant | Boîtier |
|---|---|---|
| RL_TAIL | Résistance de charge simulée, ~2-3 Ω, **5 W** | Traversant, footprint `Resistor_THT:R_Axial_Power_L25.0mm_W9.0mm_P27.94mm` |
| RSH_TAIL | Résistance shunt, ~0,05-0,1 Ω, **5 W** | Traversant, même footprint que RL_TAIL |
| U_TAIL_A/B/C | 3/4 de LM339 (comparateur quadruple — 3 des 4 comparateurs internes utilisés) | DIP-14, socket turned-pin (R7) |
| OPTO_TAIL | PC817-2 | DIP-8 traversant |
| Q_TAIL_INJECT (Q11) | IRLZ44N | TO-220 |
| POT_TAIL (R23/R24/R25) | Potentiomètre de calibration ×3 (injection/ouvert/court) | Traversant, multi-tours |
| R37 | 100 kΩ — bras haut du diviseur anti-oscillation (voir note ci-dessous) | Traversant |
| R38 | 10 kΩ — bras bas du diviseur anti-oscillation, vers `GND` | Traversant |

**Note de dimensionnement (RL_TAIL) :** 5 W jugé suffisant pour le canal TAIL (feu de position, courant nominal faible, ~0,5-1 A), avec une marge raisonnable. **À revalider canal par canal** — les canaux à fort courant (BRAKE, AUX — frein électrique) nécessiteront probablement une puissance supérieure une fois le courant nominal réel mesuré (§10.2, §10.5, point ouvert). Décision délibérée de ne pas surdimensionner tous les canaux par précaution générale (R2/R3 — la marge doit être justifiée par un calcul réel, pas ajoutée par défaut si ça alourdit inutilement un boîtier qui se veut compact).

**✅ Résolution §10 — oscillation/chatter sur Q_TAIL_INJECT (option B retenue) :** le point de mesure d'U2A (comparateur d'injection) a été déplacé **avant** Q11 (côté `Veh_Tail`, en amont) plutôt que sur le shunt R22 (situé après Q11). Raison : dès que Q11 coupe, le courant à travers R22 tombe à zéro peu importe si le défaut externe persiste ou non (loi de Kirchhoff — un courant en série est nul partout dès qu'un seul point du chemin est ouvert) ; un shunt après Q11 ne peut donc jamais détecter un défaut qui persiste une fois Q11 coupé, ce qui causait le rebasculement immédiat (chatter). La solution retenue mesure une **tension**, pas un courant, via un pont diviseur à haute impédance (R37 100 kΩ / R38 10 kΩ) posé directement sur `Veh_Tail`, indépendant de la continuité du courant à travers Q11. U2A pin 5 (+) est rebranché sur le point milieu du diviseur (au lieu du shunt R22). **Comportement résultant :** tant qu'une tension anormale persiste en amont (panne interne réelle), Q11 reste coupé en continu, sans aucun pulse de courant répété vers le véhicule ; le circuit se réarme automatiquement, sans intervention manuelle, dès que la tension anormale disparaît réellement. U2B/U2C (ouvert/court) restent inchangés, toujours branchés sur le shunt R22.

**Correction de câblage associée :** Q11 pin 3 (source) était débranché (trouvé lors de la vérification par capture d'écran) — reconnecté à l'étiquette **locale** `Q11_Source` (R6), qui rejoint R21 (3 Ω/5W) puis R22 (shunt) vers `GND`, rétablissant le chemin de courant complet du canal.
...
**État du câblage (en cours — non figé) :**

Réf. KiCad confirmées par capture d'écran : U3 (LM339, sections B=Ouvert/C=Court, A/D neutralisées comme U2), SW3 (SW_SPST), F2 (Polyfuse), Q12 (IRLZ44N), D25 (TVS bidirectionnel), R34 (shunt, ~0,05-0,1 Ω / 1-2 W — voir §10.1). R31/R32 = potentiomètres de calibration (POT2_TAIL).

Table de câblage proposée (patte à patte), **en attente de confirmation finale** :

| Sort de → | Entre dans → | Rôle |
|---|---|---|
| POT2_TAIL (R31) curseur | U3.7 (U3B +) | Calibration Seuil_Ouvert |
| POT2_TAIL (R32) curseur | U3.11 (U3C +) | Calibration Seuil_Court |
| RSH2_TAIL (R34), borne haute | U3.6 (U3B −) et U3.10 (U3C −) | Mesure tension shunt, même nœud pour les 2 comparateurs |
| U3.1 (sortie U3B, Ouvert) | pull-up vers `V_Ref` + étiquette hiérarchique `Ouvert_Tail_Trailer` | Vers Bloc_Test_Tail |
| U3.13 (sortie U3C, Court) | pull-up vers `V_Ref` + étiquette hiérarchique `Court_Tail_Trailer` | Vers Bloc_Test_Tail |
| U3.9 (U3D +) → `GND` ; U3.8 (U3D −) → `V_Ref` ; U3.14 non connectée | — | Section D neutralisée, comme U2D |
| U3.3 (V+) → `V_Ref` ; U3.12 (GND) → `GND` | — | Alimentation puce |
| `V12_Rail` → F2.1 → F2.2 → Q12.2 (Drain) → Q12.3 (Source) → RSH2_TAIL (R34, borne haute) | — | Chemin de puissance |
| RSH2_TAIL (R34, borne basse) | Étiquette hiérarchique `Trail_Tail` | Vers connecteur physique TRAILER (racine) |
| Nœud `Trail_Tail` ↔ `GND` | D25 (TVS) | Protection transitoires côté remorque |
| `V_Ref` → SW3.1 ; SW3.2 → Q12.1 (Gate) + pull-down → `GND` | Étiquette hiérarchique `Btn_Tail_Etat` sur ce nœud | Commande du MOSFET par le bouton |

**⚠️ Anomalie de valeurs en cours de résolution (non résolue à cette version) :**
- R35 = **4,7 kΩ** (et non 10 kΩ comme noté dans un brouillon précédent)
- R36 = **10 kΩ** — rôle probable : pull-down sur le nœud Gate Q12 / `Btn_Tail_Etat` (à confirmer)
- R37 = rôle à confirmer (pull-up sortie comparateur, valeur probable 4,7 kΩ)
- Une résistance supplémentaire (valeur ~4,7 kΩ) a été découverte physiquement cachée sous un autre composant sur la feuille ; référence, connexions et rôle **non identifiés** — à documenter avant de considérer le sous-bloc câblé.

Tant que ces 4 résistances (R35, R36, R37, + la résistance cachée) ne sont pas confirmées une à une (référence, valeur, connexions), la table ci-dessus reste une **proposition**, pas un état câblé validé.

#### 7.2.4 Composants — Bloc de Test (logique d'affichage, canal TAIL)

> Références réelles telles que générées par KiCad. Circuit complet, câblé et validé.

**Étage 1 — Portes intermédiaires (combine VEHICLE/TRAILER selon le mode actif)**
...
**Étage 2 — Priorité rouge et activation finale des 3 couleurs**

| Réf. KiCad | Composant | Rôle |
|---|---|---|
| D18, D19 | 1N4148 | Porte OU → nœud `Ouvert_Ou_Bleu` (`Ouvert_Tail` OU `Bleu_Tail`) |
| R14 | 4,7 kΩ, pull-down vers `GND` | Nœud `Ouvert_Ou_Bleu` |
| Q9 | 2N7000 | Gate = `Court_Tail` ; Drain = nœud `Ouvert_Ou_Bleu` ; force ce nœud à `GND` si court-circuit (priorité rouge absolue sur le bleu) |
| D20, D21, D22 | 1N4148 | Porte OU (3 entrées) → nœud `Fault_Actif` (`Court_Tail` OU `Ouvert_Tail` OU `Bleu_Tail`) |
| R15 | 4,7 kΩ, pull-down vers `GND` | Nœud `Fault_Actif` |
| Q10 | 2N7000 | Inverseur : Gate = `Fault_Actif` ; Drain = nœud `Vert_Enable` |
| R18 | 4,7 kΩ, pull-up vers `V_Ref` | Nœud `Vert_Enable` |
| D23, D24 | 1N4148 | Porte OU → nœud `Rouge_Enable` (`Court_Tail` OU `Ouvert_Tail`) |
| R19 | 4,7 kΩ, pull-down vers `GND` | Nœud `Rouge_Enable` |

**Étage 3 — Pilotage des transistors de couleur et DEL**

| Réf. KiCad | Composant | Rôle |
|---|---|---|
| Q6 | 2N7000 | Gate = `Rouge_Enable` ; Drain → R17 (330 Ω) → DEL cathode Rouge ; Source → `GND` |
| Q7 | 2N7000 | Gate = `Vert_Enable` ; Drain → R16 (330 Ω) → DEL cathode Verte ; Source → `GND` |
| Q8 | 2N7000 | Gate = nœud `Ouvert_Ou_Bleu` (après override Q9) ; Drain → R7 (330 Ω) → DEL cathode Bleue ; Source → `GND` |
| R16, R17, R7 | 330 Ω | Limitatrices de courant DEL (une par couleur) |
| LED_ARGB (D26) | DEL RGB **anode commune** (correction — voir Annexe A.3) | Anode commune → `V_Ref` ; 3 cathodes pilotées individuellement par Q6/Q7/Q8 |
...
**Correction importante (Annexe A.3) :** la DEL RGB est à **anode commune**, pas à cathode commune comme initialement noté à l'Annexe A — corrigé dans cette mise à jour. Ce choix est requis pour rester cohérent avec le pilotage bas-côté (low-side) par Q6/Q7/Q8 déjà utilisé partout ailleurs dans le projet, sans avoir à introduire des transistors haut-côté (high-side) additionnels.

**Point résolu (pour l'historique) :** les diodes des 5 portes ET avaient d'abord été posées avec l'orientation d'une porte OU par erreur — corrigé (voir règle de câblage ci-dessus). La logique de priorité rouge et le pilotage final des 3 transistors R/V/B sont maintenant câblés et validés (§7.2.4, étages 2 et 3).

---

### 7.3 Canal LEFT — ouverture (reprend le modèle Canal_TAIL, §7.2)

> **Statut :** le Canal_TAIL est **✅ validé et clos** (§7.2) et sert de modèle figé. Pour LEFT : **VEHICLE_LEFT ✅ validé** (références réelles ci-dessous, confirmées par export BOM — remplace la numérotation provisoire de la v22). **TRAILER_LEFT** : références réelles réservées et confirmées par BOM (ci-dessous), câblage patte-à-patte pas encore posé dans KiCad. **Bloc_Test_Left** : pas commencé (objet de la présente conversation).

**Structure retenue (identique à TAIL, §7.1/§7.2) :** `Canal_LEFT` contient 3 sous-feuilles imbriquées : `VEHICLE_LEFT`, `TRAILER_LEFT`, `Bloc_Test_Left`.

**Nomenclature des signaux internes (canal LEFT, par analogie avec §7.2.1) :** mêmes types d'étiquette que pour TAIL, suffixe `_LEFT` remplaçant `_TAIL` :

| Signal | Type d'étiquette | Sous-bloc |
|---|---|---|
| `VEH_LEFT` | Hiérarchique | VEHICLE_LEFT ↔ Canal_LEFT ↔ page racine |
| `TRAIL_LEFT` | Hiérarchique | TRAILER_LEFT ↔ Canal_LEFT ↔ page racine |
| `Court_Left_Vehic`, `Ouvert_Left_Vehic` | Hiérarchique | VEHICLE_LEFT → Bloc_Test_Left |
| `Court_Left_Trailer`, `Ouvert_Left_Trailer`, `Btn_Left_Etat` | Hiérarchique | TRAILER_LEFT → Bloc_Test_Left |
| `MODE_VEHICULE`, `MODE_TRAILER`, `V_REF`, `GND` | Globale (établies au Bloc 1/2) | Tous |

#### 7.3.1 Composants — sous-bloc VEHICLE (canal LEFT) — ✅ validé, références réelles (BOM)

| Rôle | Réf. réelle | Composant | Boîtier |
|---|---|---|---|
| Résistance de charge | R64 | 3 Ω, 5 W | Traversant, `R_Axial_Power_L25.0mm_W9.0mm_P27.94mm` |
| Résistance shunt | R65 (RSH_Left) | 0,05 Ω, 5 W | Traversant, même footprint |
| Comparateur triple | U4 (LM339) | Sections = injection/ouvert/court | DIP-14, socket turned-pin (R7) |
| Isolement | OC3, OC4 (PC817) | 2 canaux isolés (Ouvert/Court) — 2 boîtiers DIP-4, comme OC1/OC2 pour TAIL | DIP-4 traversant ×2 |
| MOSFET injection | Q19 (Q_Left_Inject, IRLZ44N) | — | TO-220 |
| Potentiomètres de calibration | R61 (Pot_Left_Inject), R62 (Pot_Left_Ouvert), R63 (Pot_Left_Court) | Seuils injection/ouvert/court (Option B, indépendants) | Traversant, multi-tours |
| Pull-ups sorties comparateur | R54 (PU_A), R55 (PU_B), R56 (PU_C), 4,7 kΩ | — | Traversant |
| Limitatrices DEL (opto) | R57 (LED_Court), R58 (LED_Ouvert), 1 kΩ | — | Traversant |

*(Note : le rôle précis A/B/C = injection/ouvert/court par section de U4, et le mapping exact R54/R55/R56 → quelle sortie comparateur, restent à confirmer patte-à-patte sur la feuille KiCad réelle avant de les considérer câblés — cette table ne fait que fixer les références, pas encore les connexions.)*

#### 7.3.2 Composants — sous-bloc TRAILER (canal LEFT) — références réelles réservées (BOM), câblage à venir

Par analogie avec TRAILER_TAIL (§7.2.3) :

| Rôle (par analogie TAIL) | Réf. réelle (LEFT) | Équivalent TAIL |
|---|---|---|
| MOSFET de sortie | Q20 (Q_Left_Inject_Trailer, IRLZ44N) | Q12 |
| Comparateur double (ouvert/court) | U5 (LM339) | U3 |
| Polyfuse | F3 | F2 |
| Switch bouton-poussoir | SW4 (SW_SPST) | SW3 |
| TVS bidirectionnelle | D50 | D25 |
| Résistance shunt (RSH2_Left) | R67 | R34 |
| Potentiomètres de calibration ×2 (Ouvert, Court) | R68, R69 | R31, R32 |
| Pull-ups sorties comparateur ×2 | R70, R71 | équivalents à R35/R37 (rôle pull-up, non confirmés individuellement sur TAIL) |
| Pull-down (nœud Gate Q20 / `Btn_Left_Etat`) | R72 | équivalent à R36 |

**⚠️ Deux points à vérifier avant de câbler (hérités de l'ambiguïté non résolue sur TAIL, §7.2.3) :**
- Sur TAIL, R35/R36/R37 + une 4ᵉ résistance cachée (non identifiée) ont été trouvées sur la feuille réelle — leur rôle exact n'a jamais été confirmé un à un, malgré la fermeture du canal TAIL. La liste ci-dessus pour LEFT (R70/R71/R72, 3 résistances) suppose que le besoin réel est de 3 (2 pull-ups + 1 pull-down), pas 4 — **à confirmer sur la feuille KiCad réelle**, pas à assumer.
- **OC5 n'apparaît pas dans le tableau ci-dessus.** Le modèle TRAILER_TAIL (§7.2.3, netlist) ne montre **aucun optocoupleur** côté TRAILER — les signaux `Court_Tail_Trailer`/`Ouvert_Tail_Trailer` vont directement du comparateur U3 aux portes à diodes du Bloc_Test_Tail, sans isolement (logique : le côté TRAILER est la propre source active de l'appareil, pas une entrée externe à isoler). Une référence OC5 a néanmoins été relevée comme prochaine disponible dans le BOM — **rôle non confirmé, possiblement réservée pour un autre bloc (ex. test de masse, §A.4) plutôt que pour TRAILER_LEFT.** Ne pas assumer son usage ici avant vérification.

*(Bloc_Test_Left à détailler une fois TRAILER_LEFT câblé et validé, par symétrie avec §7.2.4.)*
...
| Jauge de batterie | LM3914 (driver bar/dot analogique) + bargraph LED 10 segments OSX10201-GYR1 (2 rouge, 3 jaune, 5 vert), potentiomètre de calibration Bourns 3296W-1-102LF (1 kΩ, 25 tours) — **✅ fermé**, voir Annexe A.6 |
...
7. **Retour de masse commun (GND, pin1 GX16-10) — courant cumulé multi-canaux.** Plusieurs canaux peuvent être actifs simultanément côté VEHICULE (ex. TAIL + BRAKE + clignotant, scénario réaliste et fréquent). Le courant dans le retour de masse commun est la **somme** des canaux actifs, pas le pire cas d'un seul canal. À vérifier avant fabrication : (a) calibre de courant de la broche 1 du GX16-10 vs somme cumulée réaliste, (b) section de piste/fil du retour de masse commun, (c) topologie en étoile vers un point unique plutôt qu'en guirlande, pour éviter un « ground bounce » entre shunts de canaux différents (risque accru avec des shunts aussi bas que 0,05 Ω). **Conversation dédiée requise, pas encore commencée.**

**État des chantiers transversaux (session du 11 septembre 2026, rattrapage documentaire) :**

| Chantier | Statut |
|---|---|
| Injection vers véhicule (F5, 6 canaux) — étiquettes hiérarchiques `Injection_x_Fault` | ✅ Fermé |
| Alarme court-circuit bus (RSH_BUS, Q55/Q56/Q57) | ✅ Fermé |
| Hub / Alerte Incohérence (Mode_Coherence → Alarmes_Buzzer) | ✅ Fermé |
| Coupure V12_Rail sur défaut bus (Q64/Q65) | ✅ Conçu et câblé |
| Bloc_Test_Left | ✅ Validé (22/22 diodes) |
| Nommage Bloc_Test_Brake (suffixes de nœuds locaux) | ✅ Fermé |
| ERC global | 8 erreurs → 0 (13 avertissements résiduels, classés bénins) |

### 10.1 Checklist de revalidation par canal (composants dimensionnés empiriquement — R8 / Chantier #7)

> À exécuter pour chacun des 6 canaux avant fabrication, une fois le courant nominal réel mesuré. Tant qu'une ligne n'est pas cochée, la valeur documentée ailleurs (Annexe A, §7.2.x) reste une **estimation de brouillon**, pas une valeur finale.

**🔴 CONFLIT NON RÉSOLU — deux approches concurrentes développées séparément, aucune des deux n'est officiellement retenue :**

| | Approche A (session 11 sept.) | Approche B (session ultérieure) |
|---|---|---|
| Principe RL_x | **Valeur commutable**, switch global unique (2 positions : incandescent / DEL), 5 canaux d'éclairage | Valeur fixe unique, dimensionnée pour le « pire cas marché » |
| TAIL | 0,6 A (incand.) / 0,15 A (DEL) | 5 A → RL=2,7Ω/75W, RSH=0,05Ω/5W×2, PTC~7A/14A |
| BRAKE | 2,1 A (incand.) / 0,3 A (DEL) | 6,27 A → RL=2,2Ω/100W, RSH=0,05Ω/5W, PTC~9A/18A |
| LEFT/RIGHT/BACKUP | 2,1 A (incand.) / 0,3 A (DEL) | 5,11 A → mêmes valeurs que TAIL |
| AUX | 2-12 A — **question ouverte** (freinage léger vs fort), exclu du switch (électroaimant, valeur fixe dédiée) | Tandem/2 essieux retenu (~16 A) → 4×3,3Ω/75W parallèle, RSH=0,01Ω/5W ; triple essieu suspendu en version future |

**Ce conflit doit être tranché avant de considérer une quelconque valeur ci-dessus comme finale.** Les deux approches ne sont pas de simples variantes d'arrondi — elles impliquent un composant supplémentaire (switch global) ou non, et des courants de dimensionnement différents d'un facteur ~8-10×.

| Composant | Calcul requis | Statut (TAIL) | Statut (BRAKE/LEFT/RIGHT/BACKUP/AUX) |
|---|---|---|---|
| RL_x (résistance de charge, côté VEHICLE) | Valeur selon charge DEL/incandescente réelle du canal | 🔴 Conflit — voir tableau ci-dessus | 🔴 Conflit |
| RSH_x (shunt, côté VEHICLE) | P = I²×R avec courant nominal mesuré + marge | 🔴 Dépend de l'arbitrage RL_x | 🔴 Dépend de l'arbitrage RL_x |
| RSH2_x (shunt, côté TRAILER) | P = I²×R avec courant nominal mesuré + marge | 🔴 Dépend de l'arbitrage RL_x | 🔴 Dépend de l'arbitrage RL_x |
| PTC_x (polyfuse) | Calibre courant de maintien vs courant nominal + marge | 🔴 Dépend de l'arbitrage RL_x | 🔴 Dépend de l'arbitrage RL_x |
| TVS_x | Tenue en tension vs transitoires attendues (véhicule/remorque) | ⏳ En attente (indépendant du conflit RL_x) | ⏳ En attente |
...
| DEL RGB (**anode commune** — correction, §7.2.4) | Générique traversant 5 mm ou rectangulaire, RGB anode commune, montage direct en façade (R5) | 1 | 6 | Affichage Bleu/Vert/Violet/Rouge (§5.0) ; pilotage bas-côté par Q6/Q7/Q8 |
| Transistor de commutation (pilotage DEL R/V/B) | 2N7000, boîtier TO-92 traversant (R5) | 3 | 18 | Un par couleur (R/V/B), tout-ou-rien |
| Transistor inverseur/priorité (Bloc de Test) | 2N7000, boîtier TO-92 traversant (R5) | 3 | 18 | Inversion `BTN_TAIL_ETAT` (Q5) + inversion `Fault_Actif`→Vert (Q10) + override priorité rouge sur bleu (Q9) (§7.2.4) |
| Diode logique 1N4148 (Bloc de Test) | 1N4148, traversant (R5) | 22 | 132 | Portes ET/OU à diodes (7 portes intermédiaires + 3 portes de sortie R/V/B), combine VEHICLE/TRAILER/bouton (§7.2.4) |
| Résistance de tirage (Bloc de Test) | 4,7 kΩ, traversant 1/4 W (R5) | 13 | 78 | Pull-up/pull-down des nœuds logiques des portes ET/OU (§7.2.4) |
| Résistance limitatrice DEL (Bloc de Test) | 330 Ω, traversant 1/4 W (R5) | 3 | 18 | Limite le courant de chaque couleur de la DEL RGB (§7.2.4) |
...
### A.6 Jauge de batterie (10 segments) — ✅ FERMÉ

> Construite dans `Alimentation.kicad_sch` (et non `Alarmes_Buzzer_sch` comme envisagé initialement en cours de projet). `Batt_Interne_Pos` est une étiquette **locale** de cette feuille (pas de promotion en hiérarchique nécessaire, la jauge étant sur la même feuille que le pack/BMS).

| Réf. | Composant | Valeur | Boîtier |
|---|---|---|---|
| U17 | LM3914N | — | DIP-18, **socket turned-pin** (R7) |
| D25 | OSX10201-GYR1 | 10 segments, 5 vert / 3 jaune / 2 rouge | Traversant, 20 broches |
| R36 | Résistance (R1 réseau réf.) | 1,5 kΩ | Traversant 1/4 W |
| R53 | Résistance (R2 réseau réf.) | 3,9 kΩ | Traversant 1/4 W |
| R232 | Résistance (diviseur haut) | 18 kΩ | Traversant 1/4 W |
| R233 | Résistance (diviseur bas) | 10 kΩ | Traversant 1/4 W |
| VR1 | Potentiomètre Bourns 3296W-1-102LF | 1 kΩ, 25 tours, pattes 2+3 pontées (convention projet) | Traversant, multi-tours |
| R234 | Résistance (R_VLed, limitation thermique bus LED) | 68 Ω | Traversant, **5 W** (générique céramique/bobinée) |
| C6 | Condensateur céramique (découplage bus LED) | 0,1 µF | Traversant, **en parallèle** VLed_Jauge↔GND (pas en série) |

**Topologie de référence (span fixe) :** RHI (pin6) ↔ REF OUT (pin7) lien direct ; RLO (pin4) ↔ REF ADJ (pin8) lien direct ; MODE (pin9) ↔ V+ (pin3) pour le mode bar. Entrée SIG (pin5) alimentée par le diviseur R232/R233/VR1 depuis `Batt_Interne_Pos`, plage couverte ≈ 9,0-12,6 V (pleine plage BMS pack 3S Li-ion).

**Mapping LED du LM3914 vers le bargraph** (anode commune bargraph = broches 1-10 vers `VLed_Jauge` via R234 ; cathodes = broches 11-20) :

| Sortie LM3914 | Broche cathode bargraph | Couleur |
|---|---|---|
| LED1 (pin1) | 11 (segment J) | Rouge — seuil bas |
| LED2 (pin18) | 12 (segment I) | Rouge |
| LED3 (pin17) | 13 (segment H) | Jaune |
| LED4 (pin16) | 14 (segment G) | Jaune |
| LED5 (pin15) | 15 (segment F) | Jaune |
| LED6 (pin14) | 16 (segment E) | Vert |
| LED7 (pin13) | 17 (segment D) | Vert |
| LED8 (pin12) | 18 (segment C) | Vert |
| LED9 (pin11) | 19 (segment B) | Vert |
| LED10 (pin10) | 20 (segment A) | Vert — seuil haut |
