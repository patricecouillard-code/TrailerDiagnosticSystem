...
**Version :** 21.0 — Ouverture du Canal_LEFT (§7.3) : structure et liste de composants VEHICLE_LEFT proposées, calquées sur le modèle Canal_TAIL (§7.2) ; numérotation des références et statut final du Canal_TAIL restent en attente de confirmation
**Date :** Août 2026
...
> Le canal TAIL sert de modèle complet. Les 5 autres canaux (BRAKE, LEFT, RIGHT, BACKUP, AUX) reprendront exactement la même structure — seule la référence change (ex. `_BRAKE` au lieu de `_TAIL`). **État au moment de cette mise à jour :** le **Bloc de Test** est câblé, validé, et déplacé dans sa sous-feuille `Bloc_Test_Tail` (structure Option 2, §7.1). Le sous-bloc **VEHICLE** est en cours de construction dans `VEHICLE_TAIL`. Le sous-bloc **TRAILER** reste à construire dans `TRAILER_TAIL`.
...
> **Statut à l'ouverture de cette section :** deux points restaient en suspens au moment de commencer LEFT et n'ont pas encore été confirmés par l'utilisateur :
> 1. **État du Canal_TAIL** — `TRAILER_TAIL` n'était pas terminé (R35/R36/R37 en attente de câblage) et le point §10 (risque d'oscillation/chatter sur `Q_TAIL_INJECT`) restait ouvert. À clarifier si TAIL est maintenant clos ou si ces points restent actifs en parallèle.
> 2. **Numérotation des références KiCad** — aucune confirmation reçue sur les prochaines références libres (après R21–R37, U2–U3, Q11–Q12, etc. utilisés par TAIL). La numérotation ci-dessous est une proposition provisoire, à corriger une fois l'état réel du projet KiCad connu.
